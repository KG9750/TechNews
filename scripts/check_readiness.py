...
POST_READINESS_READY_MVP_ISSUES = set(range(10, 21))
POST_READINESS_NEEDS_TRIAGE_MVP_ISSUES = set()
COMPLETED_MVP_ISSUES = set(range(10, 21))
MVP_ISSUE_DRAFTS = {
...
def check_readiness_action_packet_helper() -> list[str]:
    ...
    for needle in [
        ...
        "source-owner-reviews/mvp-source-set-narrowing.md",
        "mvp-issue-packets",
        ...
        "github_live_spike_issue_comment_sections",
        "spike_packet_path_for_env_group",
    ...
    for needle in [
        ...
        "Spike packet: evidence/live-spike-packets/archive-storage.md",
        "Required evidence files",
        "External Input Request Checklist",
...
def check_mvp_issue_drafts() -> list[str]:
    ...
    for needle in [
        ...
        "After the readiness gate passes, #10 through #20 can move to `ready-for-agent`.",
    ]:
...
def check_github_tracker(evidence_root: Path = DEFAULT_EVIDENCE_ROOT) -> list[str]:
    ...
    for number in MVP_ISSUES:
        ...
        if number in COMPLETED_MVP_ISSUES:
            require(issue["state"] == "CLOSED", f"GitHub issue #{number} must be closed after its MVP work completes")
            require("needs-triage" not in labels, f"GitHub issue #{number} must not keep needs-triage after completion")
            continue
        require(issue["state"] == "OPEN", f"GitHub issue #{number} must remain open before its MVP work completes")
    ...
    return [
        ...
        (
            "GitHub tracker: #10-#20 are complete"
            if readiness_passed
