...
- `scripts/source_owner_review_decision.py` can list open source reviews, report draft status, draft ignored owner-decision files, generate ignored review packets, a packet index, and a consolidated owner worksheet, refresh current artifact context in existing drafts, validate completed decisions, and apply validated decisions to the review matrix, access policy, owner queue, and registry notes.
