...
evidence/github-update-packet.md
...
Use `evidence/source-owner-reviews/mvp-source-set-narrowing.md` when the owner wants to resolve #21 by narrowing the MVP source set. It lists the current production-enabled metadata-only sources, coverage gaps, unresolved batches, and the decision rules for marking excluded sources `deferred` or `blocked`. It is context only; the JSON decision files remain the source of truth.
...
Use `evidence/delivery-schedule-decision.md` to collect the owner decision questions and validation commands without recording the actual time in tracked files or GitHub.
...
- User and group `data.message_id` values must differ; if message ids are redacted, use distinct stable labels such as `REDACTED_MESSAGE_ID_USER` and `REDACTED_MESSAGE_ID_GROUP`.
