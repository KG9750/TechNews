...
Before live execution, run `scripts/spikes/live_readiness_preflight.py --dry-run --write-packet` and `scripts/readiness_action_packet.py` to aggregate helper dry-runs, missing environment variable names, missing evidence files, source-owner review status, and next commands without printing secret values.
