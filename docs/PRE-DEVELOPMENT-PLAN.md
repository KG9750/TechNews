...
Before live execution, run `scripts/spikes/live_readiness_preflight.py --dry-run --write-packet --write-spike-packets` and `scripts/readiness_action_packet.py` to aggregate helper dry-runs, missing environment variable names, missing evidence files, live evidence validation failures, per-spike issue-facing packets, source-owner review status, source-owner batch plan, the MVP issue unlock matrix, and next commands without printing secret values.
