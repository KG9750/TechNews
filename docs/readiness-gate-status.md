...
Pre-development readiness is also protected in GitHub Actions by `.github/workflows/pre-development-readiness.yml`. The workflow opts JavaScript actions into the Node 24 runtime path, runs the local readiness checker, compiles the readiness/spike scripts, and proves the tracked evidence templates cannot pass as live evidence. It intentionally does not require live external credentials.
...
| Pre-development readiness CI exists | Passed | `.github/workflows/pre-development-readiness.yml`; `scripts/check_readiness.py` verifies Node 24 action runtime opt-in, workflow coverage, secrets inventory alignment, live evidence helper coverage, and local fixtures | None |
...
| Live evidence runbook and template-negative gate exist | Passed | `docs/live-spike-evidence-runbook.md`; `scripts/spikes/live_readiness_preflight.py`; `scripts/spikes/readiness_manifest.py`; `scripts/test_live_evidence_helpers.py`; `fixtures/live-evidence-templates/`; `fixtures/live-evidence-templates/readiness-manifest.json`; `fixtures/live-evidence-negative/leaky-feishu/`; `scripts/check_readiness.py --require-evidence --evidence-root fixtures/live-evidence-templates` fails on `TEMPLATE_` placeholders; live evidence scans for common token/id/path leaks and raw sensitive environment values; preflight reports missing env/evidence without secret values; final evidence manifest checks declared files and model/archive run metadata; Feishu runner redaction, helper regressions, and a synthetic redacted positive evidence package are covered by readiness | None |
