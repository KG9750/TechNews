...
- Source eligibility: 32 first-version seed sources are covered in `docs/source-eligibility-reviews.md`; all first-version sources now have source-specific evidence or per-URL review rules recorded. 7 metadata/text-only sources are `eligible`, and 25 public/company/manual sources remain `needs_review` before automated production ingestion.
...
| Source eligibility review covers first-version seed sources | Review | `docs/source-eligibility-reviews.md`; 32 first-version sources covered, 7 `eligible`, 25 `needs_review`; no first-version source remains without recorded evidence or per-URL review rules | Owner must complete source-specific terms/media/rate-limit review before automated production ingestion for `needs_review` sources |
