...
The live spike evidence procedure is now documented in `docs/live-spike-evidence-runbook.md`. Template evidence files live under `fixtures/live-evidence-templates/`; they are examples only, and `scripts/check_readiness.py` rejects any live evidence that still contains `TEMPLATE_` placeholders or common sensitive leak patterns. Final live evidence must also include `evidence/readiness-manifest.json`, which declares the evidence set and checks model/archive run metadata consistency; `scripts/spikes/readiness_manifest.py` generates that manifest from the redacted evidence files. `scripts/spikes/live_readiness_preflight.py` provides a one-command preflight that runs local helper dry-runs, validates model-provider request envelopes as metadata-only, reports missing environment variable names, lists missing evidence files, inventories dry-run artifacts separately from final evidence, runs the same live evidence validator used by the final gate, and can write ignored Markdown execution packets without printing secret values, including per-spike packets for Feishu, model-provider, and archive-storage issue updates. `scripts/readiness_action_packet.py` then links the live packet, per-spike packets, readiness-manifest blocker, GitHub issue links, source-owner review index, source-owner worksheet, source-owner batch plan, an MVP issue unlock matrix, and ignored per-MVP-issue triage packets into one top-level execution path. A synthetic leaky fixture under `fixtures/live-evidence-negative/leaky-feishu/` proves the redaction scanner fails unsafe evidence. The Feishu spike runner also redacts common sensitive response keys and values before writing evidence, documents a custom group bot fallback shape, and keeps fallback evidence separate from the required internal-app user and group gate.
